...
async def assemble_video(tool_context: ToolContext) -> str:
    ...
    try:
        ...
        print(f"DEBUG content_planner_output type: {type(content_planner_output)}")
        print(f"DEBUG content_planner_output value: {repr(content_planner_output)[:300]}")
        if isinstance(content_planner_output, str):
            # Strip markdown code block if present
            cleaned = content_planner_output.strip()
            if cleaned.startswith("```"):
                cleaned = re.sub(r"^```[a-z]*\n?", "", cleaned)
                cleaned = re.sub(r"\n?```$", "", cleaned).strip()
            content_planner_output = json.loads(cleaned)
        ...
        # Find image and audio files matching current content plan scene IDs
        ...
        if not ffmpeg_bin:
            return "ffmpeg not found. Please install ffmpeg and ensure it is in your PATH."
